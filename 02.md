...
cor(tolerance[ , 2:6]) %>%
  ...
  mutate(row_order = rep(5:1, times = 5)) %>%
...
And if all you wanted was the lower diagonal, you could use the `lowerCor()` function from the [**psych** package](https://personality-project.org/r/).
...
Now simple use of `count()` or `nrow()` won't help us discover how many participants there are in the `tolerance_pp` data. One quick way is to `count()` the number of `distinct()` `id` values.
...
A fundamental skill is knowing how to convert longitudinal data in one format to the other. If you're using packages within the tidyverse, `gather()` function will get you from the person-level format to the person-period format.

``` r
tolerance %>%
  # this is the main event
  gather(age, tolerance, -id, -male, -exposure) %>% 
...
You can learn more about using the `gather()` function this way [here](https://r4ds.had.co.nz/tidy-data.html#spreading-and-gathering) and [here](https://uc-r.github.io/tidyr) and [here](https://tidyr.tidyverse.org/).

As hinted at in the above hyperlinks, the opposite of the `gather()` function is `spread()`. We can use `spread()` to convert the person-period `tolerance_pp` data to the same format as the person-level `tolerance` data.
...
tolerance_pp %>% 
  ...
  spread(key = age, value = tolerance)
```
...
So our `by_id` data object has many data sets stored in a higher-level data set. The code we used is verbose, but that's what made it human-readable. Now we have our nested tibble, we can make a function that will fit the simple linear model `tolerance ~ 1 + time` to each id-level data set. *Why use `time` as the predictor?* you ask. On page 29 in the text, Singer and Willett clarified they fit their individual models with (age − 11) in order to have the model intercepts centered at 11 years old rather than 0. If we wanted to, we could make an (age − 11) variable like so:
...
Did you notice how our `age_minus_11` variable is the same as the `time` variable already in the data set? Yep, that's why we'll be using `time` in the model. In our data, (age − 11) is encoded as `time`.

Singer and Willett used OLS to fit their exploratory models. We could do that to with the `lm()` function. Let's get frisky and fit the models as Bayesians, instead. Our primary statistical package for fitting Bayesian models will be [Paul Bürkner](https://twitter.com/paulbuerkner?lang=en)'s [**brms** package](https://github.com/paul-buerkner/brms). Let's open it up.
...
We just fit a single-level Bayesian regression model for our first participant. We saved the results as an object named `fit1`. We can return a useful summary of `fit1` with either `print()` or `summary()`. Since it's less typing, we'll use `print()`.
...
```
    ...
    ## Intercept     1.91      0.76     0.64     3.30       6240 1.00
    ## time          0.11      0.31    -0.44     0.64       5649 1.00
    ## 
    ## Family Specific Parameters: 
    ##       Estimate Est.Error l-95% CI u-95% CI Eff.Sample Rhat
    ## sigma     0.68      0.72     0.21     2.44       1651 1.01
    ## 
...
You probably heard somewhere that Bayesian statistics require priors. Since we didn't specify any, the `brm()` function did so for us. We can see what those were by pulling them out of our `fit1` object.
...
```
    ...
    ## Population-Level Effects: 
    ##           Estimate Est.Error l-95% CI u-95% CI Eff.Sample Rhat
    ## Intercept     1.15      0.41     0.47     1.82       6634 1.00
    ## time          0.18      0.18    -0.11     0.46       6155 1.00
    ## 
    ## Family Specific Parameters: 
    ##       Estimate Est.Error l-95% CI u-95% CI Eff.Sample Rhat
    ## sigma     0.37      0.45     0.11     1.31       1615 1.00
    ## 
    ## Samples were drawn using sampling(NUTS). For each parameter, Eff.Sample 
    ## is a crude measure of effective sample size, and Rhat is the potential 
    ## scale reduction factor on split chains (at convergence, Rhat = 1).

Different participant, different model results.

Looking ahead a bit, we'll need to know how to get the *R*<sup>2</sup> for a single-level Gaussian model. With brms, you do that with the `bayes_R2()` function.
...
```
    ...
    ## R2 0.6281891 0.2451232 0.01196133 0.8148495

Though the default spits out summary statistics, you can get the full posterior distribution for the *R*<sup>2</sup> by specifying `summary = F`.
...
```
    ...
    ##  num [1:16000, 1] 0.0031 0.6882 0.723 0.0858 0.7954 ...
...
![](02_files/figure-markdown_github/unnamed-chunk-24-1.png)

You'll note how non-Gaussian the Bayesian *R*<sup>2</sup> can be. Also, with only 5 data points + default minimally-informative priors in our `id`-specific model, there' massive uncertainty in the shape. As such, the value of central tendency will vary widely based on which statistic you use.
...
```
    ...
    ##        mean    median      mode
    ## 1 0.6281891 0.7488447 0.7994559
...
I should also point out the **brms** package did not get these *R*<sup>2</sup> values by traditional method used in, say, OLS estimation. To learn more about how the Bayesian *R*<sup>2</sup> sausage is made, check out the paper by [Gelman, Goodrich, Gabry, and Vehtari](https://www.tandfonline.com/doi/abs/10.1080/00031305.2018.1549100?journalCode=utas20).
...
models <- 
  by_id %>%
  mutate(model = map(data, ~update(fit1, newdata = .)))
```

Let’s walk through what we did. The `map()` function takes two primary arguments, `.x` and `.f`, respectively. We set `.x = data`, which meant we wanted to iterate over the contents in our `data` vector. Recall that each row of `data` itself contained an entire data set—one for each of the 16 participants. It’s the second argument `.f` that we indicated what we wanted to do with our rows of `data`. We set that to `.f = ~update(fit1, newdata = .)`. By the `~` syntax, we entered in a formula. That formula was `update(fit1, newdata = .)`. Just like we did with `fit2`, above`, re reused the model formula and other technical specs from`fit1`. Now notice the last part of the formula,`newdata = .`. That little`.`refers to the element we specified in the`.x`argument. What this combination means is that for each of the 16 rows of our nested`by\_id`tibble, we plugged in the`id`-specific data set into`update(fit`, newdata[[i]])` where `i` is simply meant as a row index. The results from all this is a new column, `model`, which contains the output of each of the 16 iterations.
...
```
    ...
    ##       id data             model        
    ##    <int> <list>           <list>       
    ##  1     9 <tibble [5 × 5]> <S3: brmsfit>
    ##  2    45 <tibble [5 × 5]> <S3: brmsfit>
    ##  3   268 <tibble [5 × 5]> <S3: brmsfit>
    ##  4   314 <tibble [5 × 5]> <S3: brmsfit>
    ##  5   442 <tibble [5 × 5]> <S3: brmsfit>
    ##  6   514 <tibble [5 × 5]> <S3: brmsfit>
    ##  7   569 <tibble [5 × 5]> <S3: brmsfit>
    ##  8   624 <tibble [5 × 5]> <S3: brmsfit>
    ##  9   723 <tibble [5 × 5]> <S3: brmsfit>
    ## 10   918 <tibble [5 × 5]> <S3: brmsfit>
    ## 11   949 <tibble [5 × 5]> <S3: brmsfit>
    ## 12   978 <tibble [5 × 5]> <S3: brmsfit>
    ## 13  1105 <tibble [5 × 5]> <S3: brmsfit>
    ## 14  1542 <tibble [5 × 5]> <S3: brmsfit>
    ## 15  1552 <tibble [5 × 5]> <S3: brmsfit>
    ## 16  1653 <tibble [5 × 5]> <S3: brmsfit>
...
mean_structure <-
  ...
  spread(key = coefficients, value = temp) %>% 
...
```
    ...
    ##   <int>         <dbl>        <dbl>           <dbl>          <dbl>
    ## 1     9          1.89         0.8             0.12           0.31
    ## 2    45          1.14         0.4             0.17           0.16
    ## 3   268          1.53         0.73            0.02           0.3 
    ## 4   314          1.3          0.44           -0.03           0.19
    ## 5   442          1.57         0.53            0.06           0.22
    ## 6   514          1.43         0.41            0.27           0.17

It's a little simpler to extract the residual variance. Recall that because **brms** gives that in the standard deviation metric (i.e., *σ*), you need to square it to return it in a variance metric (i.e., *σ*<sup>2</sup>).
...
Next, we'll extract our Bayesian *R*<sup>2</sup> summaries. Given how nonnormal these are, we'll use the posterior median rather than the mean. We get that by using the `robust = T` argument within the `bayes_R2()` function.
...
|-----:|----------------:|---------------:|------------------:|-----------------:|-------------------:|-----:|-----:|---------:|
|     9|             1.89|            0.80|               0.12|              0.31|                0.42|  0.34|     0|      1.54|
|    45|             1.14|            0.40|               0.17|              0.16|                0.13|  0.75|     1|      1.16|
|   268|             1.53|            0.73|               0.02|              0.30|                0.47|  0.20|     1|      0.90|
|   314|             1.30|            0.44|              -0.03|              0.19|                0.17|  0.22|     0|      0.81|
|   442|             1.57|            0.53|               0.06|              0.22|                0.29|  0.25|     0|      1.13|
|   514|             1.43|            0.41|               0.27|              0.17|                0.14|  0.86|     1|      0.90|
|   569|             1.82|            0.12|               0.05|              0.06|                0.01|  0.85|     0|      1.99|
|   624|             1.12|            0.18|               0.02|              0.07|                0.01|  0.36|     1|      0.98|
|   723|             1.27|            0.32|              -0.05|              0.13|                0.06|  0.45|     0|      0.81|
|   918|             1.01|            0.77|               0.14|              0.33|                0.59|  0.34|     0|      1.21|
|   949|             1.74|            0.66|              -0.10|              0.28|                0.40|  0.30|     1|      0.93|
|   978|             1.04|            0.92|               0.63|              0.39|                0.70|  0.86|     1|      1.59|
|  1105|             1.53|            0.50|               0.16|              0.21|                0.18|  0.66|     1|      1.38|
|  1542|             1.20|            0.56|               0.24|              0.24|                0.24|  0.75|     0|      1.44|
|  1552|             1.16|            1.01|               0.16|              0.41|                0.92|  0.30|     0|      1.04|
|  1653|             0.96|            0.40|               0.25|              0.18|                0.14|  0.84|     0|      1.25|
...
![](02_files/figure-markdown_github/unnamed-chunk-34-1.png)
...
```
    ...
    ## Population-Level Effects: 
    ##           Estimate Est.Error l-95% CI u-95% CI Eff.Sample Rhat
    ## Intercept     1.36      0.09     1.18     1.54      16620 1.00
    ## time          0.13      0.04     0.06     0.20      16478 1.00
    ## 
    ## Family Specific Parameters: 
    ##       Estimate Est.Error l-95% CI u-95% CI Eff.Sample Rhat
    ## sigma     0.47      0.04     0.40     0.55      16084 1.00
    ## 
    ## Samples were drawn using sampling(NUTS). For each parameter, Eff.Sample 
    ## is a crude measure of effective sample size, and Rhat is the potential 
    ## scale reduction factor on split chains (at convergence, Rhat = 1).

Before, we used `posterior_summary()` to isolate the posterior means and *S**D*s. We can also use the `fixef()` function for that.
...
```
    ...
    ##            Estimate  Est.Error      Q2.5     Q97.5
    ## Intercept 1.3571705 0.09172600 1.1764546 1.5359988
    ## time      0.1309087 0.03694544 0.0578257 0.2037874
...
For this plot, we'll work more directly with the model formulas to plot the trajectories. We can use `init_stat_est` and `rate_chante_est` from the `mean_structure` object as stand-ins for *β*<sub>0<sub>id</sub></sub> and *β*<sub>1<sub>id</sub></sub> from our model equation

tolerance<sub>id</sub> = *β*<sub>0<sub>id</sub></sub> + *β*<sub>1<sub>id</sub></sub> ⋅ time<sub>id</sub> + *ϵ*<sub>id</sub>

All we need to do is plug in the appropriate values for `time` and we'll have the fitted `tolerance` values for each level of `id`. After a little wrangling, the data will be in good shape for plotting.
...
We’ll plot the `id`-level trajectories with those values and `geom_line()`. To get the overall trajectory, we’ll get tricky with `fixef(fit3)` and `geom_abline()`.

``` r
tol_fitted %>% 
  ggplot(aes(x = age, y = tolerance, group = id)) +
  geom_line(color = "blue", size = 1/4) +
  geom_abline(intercept = fixef(fit3)[1, 1] + fixef(fit3)[2, 1] * -11,
              slope     = fixef(fit3)[2, 1],
              color = "blue", size = 2) +
  coord_cartesian(ylim = 0:4) +
  theme(panel.grid = element_blank()) 
```

![](02_files/figure-markdown_github/unnamed-chunk-39-1.png)
...
```
    ...
    ## Population-Level Effects: 
    ##           Estimate Est.Error l-95% CI u-95% CI Eff.Sample Rhat
    ## Intercept     1.36      0.12     1.12     1.60       8685 1.00
    ## time          0.10      0.05     0.00     0.20       8530 1.00
    ## male          0.00      0.19    -0.36     0.37       7724 1.00
    ## time:male     0.07      0.08    -0.08     0.22       7381 1.00
    ## 
    ## Family Specific Parameters: 
    ##       Estimate Est.Error l-95% CI u-95% CI Eff.Sample Rhat
    ## sigma     0.47      0.04     0.40     0.55      12400 1.00
    ## 
    ## Samples were drawn using sampling(NUTS). For each parameter, Eff.Sample 
    ## is a crude measure of effective sample size, and Rhat is the potential 
    ## scale reduction factor on split chains (at convergence, Rhat = 1).
...
tol_fitted_male <-
  ...
  mutate(time = age - 11) %>% 
  mutate(tolerance = fixef(fit4)[1, 1] + 
           fixef(fit4)[2, 1] * time + 
           fixef(fit4)[3, 1] * male + 
           fixef(fit4)[4, 1] * time * male)
...
![](02_files/figure-markdown_github/unnamed-chunk-44-1.png)

Before we can do the same thing with `exposure`, we'll need to dichotomize it by its median. A simple way is with a conditional statement within the `ifelse()` function.

``` r
tolerance_pp <-
  tolerance_pp %>% 
  mutate(exposure_01 = ifelse(exposure > median(exposure), 1, 0))
```
...
```
    ...
    ## Population-Level Effects: 
    ##                  Estimate Est.Error l-95% CI u-95% CI Eff.Sample Rhat
    ## Intercept            1.39      0.12     1.16     1.63       8257 1.00
    ## time                 0.04      0.05    -0.06     0.14       7448 1.00
    ## exposure_01         -0.07      0.17    -0.41     0.26       7223 1.00
    ## time:exposure_01     0.18      0.07     0.04     0.32       6290 1.00
    ## 
    ## Family Specific Parameters: 
    ##       Estimate Est.Error l-95% CI u-95% CI Eff.Sample Rhat
    ## sigma     0.43      0.04     0.37     0.51      11469 1.00
    ## 
    ## Samples were drawn using sampling(NUTS). For each parameter, Eff.Sample 
    ## is a crude measure of effective sample size, and Rhat is the potential 
    ## scale reduction factor on split chains (at convergence, Rhat = 1).
...
``` r
tol_fitted_exposure <-
  tibble(exposure_01 = rep(0:1, each = 2),
         age  = rep(c(11, 15), times = 2)) %>% 
  mutate(time = age - 11) %>% 
  mutate(tolerance = fixef(fit5)[1, 1] + 
           fixef(fit5)[2, 1] * time + 
           fixef(fit5)[3, 1] * exposure_01 + 
           fixef(fit5)[4, 1] * time * exposure_01)

tol_fitted_exposure
```

    ## # A tibble: 4 x 4
    ##   exposure_01   age  time tolerance
    ##         <int> <dbl> <dbl>     <dbl>
    ## 1           0    11     0      1.39
    ## 2           0    15     4      1.56
    ## 3           1    11     0      1.32
...
Now make our Bayesian version of the bottom panels of Figure 2.7.
...
tol_fitted %>% 
  ...
  ggplot(aes(x = age, y = tolerance, color = factor(exposure_01))) +
  geom_line(aes(group = id),
            size = 1/4) +
  ...
  theme(panel.grid = element_blank(),
        legend.position = "none") +
  facet_wrap(~exposure_01)
```

![](02_files/figure-markdown_github/unnamed-chunk-48-1.png)

#### The relationship between ~~OLS-Estimated~~ single-level Bayesian trajectories and substantive predictors

Here are the left panels of Figure 2.8.

``` r
mean_structure %>% 
  select(id, init_stat_est, rate_chante_est) %>% 
  gather(key, value, -id) %>% 
  mutate(key = factor(key, labels = c("Fitted inital status", "Fitted rate of change"))) %>% 
  # we need to add `male` values to `tol_fitted`
  left_join(tolerance_pp %>% select(id, male),
            by = "id") %>% 
  
  ggplot(aes(x = factor(male), y = value, color = key)) +
...
![](02_files/figure-markdown_github/unnamed-chunk-49-1.png)
...
mean_structure %>% 
  ...
  ggplot(aes(x = exposure, y = value, color = key)) +
  geom_point(alpha = 1/2) +
  scale_color_viridis_d(option = "B", begin = .2, end = .7) +
...
```
    ...
    ##  [1] brms_2.8.8      Rcpp_1.0.1      forcats_0.3.0   stringr_1.4.0  
    ...
    ##  [17] codetools_0.2-15          mnormt_1.5-5             
    ##  [19] knitr_1.20                shinythemes_1.1.1        
    ##  [21] bayesplot_1.6.0           jsonlite_1.5             
    ##  [23] LaplacesDemon_16.1.1      broom_0.5.1              
    ##  [25] shiny_1.1.0               compiler_3.5.1           
    ##  [27] httr_1.3.1                backports_1.1.4          
    ##  [29] assertthat_0.2.0          Matrix_1.2-14            
    ##  [31] lazyeval_0.2.2            cli_1.0.1                
    ##  [33] later_0.7.3               htmltools_0.3.6          
    ##  [35] prettyunits_1.0.2         tools_3.5.1              
    ##  [37] igraph_1.2.1              coda_0.19-2              
    ##  [39] gtable_0.3.0              glue_1.3.1.9000          
    ##  [41] reshape2_1.4.3            cellranger_1.1.0         
    ##  [43] nlme_3.1-137              crosstalk_1.0.0          
    ##  [45] psych_1.8.4               ps_1.2.1                 
    ##  [47] rvest_0.3.2               mime_0.5                 
    ##  [49] miniUI_0.1.1.1            tidybayes_1.0.4          
    ##  [51] gtools_3.8.1              zoo_1.8-2                
    ##  [53] scales_1.0.0              colourpicker_1.0         
    ##  [55] hms_0.4.2                 promises_1.0.1           
    ##  [57] Brobdingnag_1.2-6         parallel_3.5.1           
    ##  [59] inline_0.3.15             shinystan_2.5.0          
    ##  [61] yaml_2.1.19               curl_3.2                 
    ##  [63] gridExtra_2.3             loo_2.1.0                
    ##  [65] StanHeaders_2.18.0-1      stringi_1.4.3            
    ##  [67] highr_0.7                 dygraphs_1.1.1.5         
    ##  [69] pkgbuild_1.0.2            rlang_0.3.4              
    ##  [71] pkgconfig_2.0.2           matrixStats_0.54.0       
    ##  [73] evaluate_0.10.1           lattice_0.20-35          
    ##  [75] rstantools_1.5.1          htmlwidgets_1.2          
    ##  [77] labeling_0.3              tidyselect_0.2.5         
    ##  [79] processx_3.2.1            plyr_1.8.4               
    ##  [81] magrittr_1.5              R6_2.3.0                 
    ##  [83] generics_0.0.2            pillar_1.3.1             
    ##  [85] haven_1.1.2               foreign_0.8-70           
    ##  [87] withr_2.1.2               xts_0.10-2               
    ##  [89] abind_1.4-5               modelr_0.1.2             
    ##  [91] crayon_1.3.4              arrayhelpers_1.0-20160527
    ##  [93] utf8_1.1.4                rmarkdown_1.10           
    ##  [95] grid_3.5.1                readxl_1.1.0             
    ##  [97] callr_3.1.0               threejs_0.3.1            
    ##  [99] digest_0.6.18             xtable_1.8-2             
    ## [101] httpuv_1.4.4.2            stats4_3.5.1             
