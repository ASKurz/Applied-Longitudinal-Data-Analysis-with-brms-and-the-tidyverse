...
A fundamental skill is knowing how to convert longitudinal data in one format to the other. If you're using packages within the tidyverse, the `gather()` function will get you from the person-level format to the person-period format.
...
Singer and Willett used OLS to fit their exploratory models. We could do that to with the `lm()` function and we will do a little of that in thie project. But let's get frisky and fit the models as Bayesians, instead. Our primary statistical package for fitting Bayesian models will be [Paul Bürkner](https://twitter.com/paulbuerkner?lang=en)'s [**brms** package](https://github.com/paul-buerkner/brms). Let's open it up.
...
```
    ...
    ##  [17] bridgesampling_0.6-0      mnormt_1.5-5             
