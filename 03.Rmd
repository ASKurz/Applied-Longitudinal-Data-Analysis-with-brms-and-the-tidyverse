...
If you want the residual variances (i.e., $\sigma_\epsilon$), you'd `unnest()` the `glance` column. They'll be listed in the `sigma` column.
...
where $\mathbf{0}$ is the vector of 0 means and $\mathbf{\Sigma}$ is the variance/covariance matrix. In **Stan**, and thus **brms**, we typically decompose $\mathbf{\Sigma}$ like so
...
where $p(\theta | d)$ is the posterior distribution, $p(d | \theta)$ is the likelihood (i.e., the star of maximum likelihood), $p(\theta)$ are the priors, and $p(d)$ is the normalizing constant, the probability of the data, that transforms the numerator to a probability metric.
...
$\hat{\pi}_{0i} =$ `r round(fixef(fit2)[1, 1], 2)` $+$ `r round(fixef(fit2)[3, 1], 2)`$\text{program}_i$
$\hat{\pi}_{1i} =$ `r round(fixef(fit2)[2, 1], 2)` $+$ `r round(fixef(fit2)[4, 1], 2)`$\text{program}_i$
