...
You'll note how non-Gaussian the Bayesian $R^2$ can be. Also, with only 5 data points + default minimally-informative priors in our `id`-specific model, there' massive uncertainty in the shape. As such, the value of central tendency will vary widely based on which statistic you use.
...
```{r models, cache = T, warning = F, message = F}
models <- 
  by_id %>%
...
It's a little simpler to extract the residual variance. Recall that because **brms** gives that in the standard deviation metric (i.e., $\sigma$), you need to square it to return it in a variance metric (i.e., $\sigma^2$).
...
For this plot, we'll work more directly with the model formulas to plot the trajectories. We can use `init_stat_est` and `rate_chante_est` from the `mean_structure` object as stand-ins for $\beta_0_\text{id}$ and $\beta_1_\text{id}$ from our model equation

$$
\text{tolerance}_\text{id} = \beta_0_\text{id} + \beta_1_\text{id} * \text{time}_\text{id} + \epsilon_\text{id}
$$
...
```{r fit5, cache = F, warning = F, message = F}
fit5 <-
  update(fit4, 
         newdata = tolerance_pp,
...
rm(tolerance, tolerance_pp, by_id, fit1, fit2, models, mean_structure, residual_variance, r2, table, fit3, tol_fitted, fit4, tol_fitted_male, fit5, tol_fitted_exposure)
