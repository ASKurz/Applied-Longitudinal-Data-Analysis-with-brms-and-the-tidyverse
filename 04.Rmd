...
If your compare our results with those in the "Model B" column in Table 4.1, you'll see our summary results match well with those in the text. Our $\gamma$s (i.e., 'Population-Level Effects:') are near identical. The leftmost panel in Figure 4.3 shows the prototypical trajectory, based on the $\gamma$s. A quick way to get that within our **brms** framework is with the `marginal_effects()` function. Here's the default output.
...
That last part, the contents of the second higher-level list indexed by `$ residual`, contains the contents for $\sigma_\epsilon$. On pate 100 in the text, Singer and Willett compared $\sigma_\epsilon^2$ from the first model to that from the second. We might do that like so.
...
$$
\sigma_\text{Residual_j}^2 = \sigma_0^2 + \sigma_1^2 \text{time}_j + 2 \sigma_{01} \text{time}_j + \sigma_\epsilon^2
$$
...
```{r}
post %>% 
  transmute(sigma_2_residual_j = sd_id__Intercept^2 + sd_id__age_14^2 * 0 + 2 * sd_id__Intercept * cor_id__Intercept__age_14 * sd_id__age_14 * 0 + sigma^2) %>% 
  head()
```
...
Our $\gamma$s are quite similar to those presented in the text. Our $\sigma_\epsilon$ for this model is about the same as with `fit2`. Let's practice with `marginal_effects()` to plot the consequences of this model.
...
Coding it as an integer further complicated things for the third plot returned by `marginal_effects()`, the one for the interaction of `age_14` and `coa`, $\gamma_{11}$. 
...
```{r fit3_c, cache = T, warning = F, message = F, results = "hide"}
fit3_c <-
  update(fit3,
...
Earlier we discussed how `marginal_effects()` let’s users adjust some of the output. But if you want an extensive overhaul, it’s better to save the output of `marginal_effects()` as an object and manipulate that object with the `plot()` function.
...
In this case, we end up with massive uncertainty when working with the full posteriors. This is particularly the case with the difference in $\sigma_1^2$, which is left skewed for days.
...
On page 111, Singer and Willett computed the various levels of the $\pi$s when `coa == 0` or `coa == 1`. To follow along, we’ll want to work directly with the posterior samples of `fit3`.
...
```{r fit6_fit7, cache = T, warning = F, message = F, results = "hide"}
fit6 <-
  update(fit5,
         newdata = alcohol1_pp,
...
Here we reproduce the $\gamma$s from `fit5` and compare the to the updates from `fit6` and `fit7`.
...
As do the frequentists, we Bayesian generally prefer models with smaller deviances, too.
...
The reason we multiplied the `se_diff` column (i.e., the standard errors for the difference estimates) is because you can't have negative standard errors. That's be silly. But anyway, notice that the `brm()` fits have been rank ordered with the smallest differences at the top. Each row in the output is the difference of one of the fits compared to the best-fitting fit. Since `fit4` apparently had the lowest WAIC value, it was ranked at the top. And notice how its `waic_diff` is 0. That, of course, is because $x - x = 0$. So all the other difference scores are follow the formula $\text{Difference}_x = \text{WAIC}_\{fit_x} - \text{WAIC}_\{fit_4}$. 
...
Their WAIC values and the associated standard errors are in the final two columns. And in the two before that, we get the $p_\text{WAIC}$ estimates and their standard errors. And we can get similar information for the LOO.
...
$$H_0: \gamma_{00} = 0 \; and \; \gamma_{10} = 0.$$
...
On page 125, Singer and Willett further muse: 
...
Singer and Willett concluded they could "reject the null hypothesis at any conventional level of significance" (p. 126). If we must appearl to the Null, here are the posterior means and 95% intervals for our differences.
...
More than a couple point estimates, we return full posterior distributions which we might summarize.
...
However pedagogically useful that might be, it's be very tedious. If we instead take a trajectory-centric perspective, it's more natural and efficient to work with a `fitted()` based workflow. Let's define our `nd` data.
...
Now we've warmed up, let's add in the data and the other lines so make the full version of Figure 4.7. Before we do so, we'll revisit `fitted()` one more time. Notice the return of the `re_formula = NA` argument. The trajectories in our `f_gamma_only` data frame will only be sensitive to the $\gamma$s.
