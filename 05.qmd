...
You may have noticed that warning about *divergent transitions*. We'll get to that in a bit. First focus on the parameter estimates for `sd(exper)`. Unlike in the text, our posterior mean is not 0.000. But do remember that our posterior is parameterized in the $\sigma$ metric. Let's do a little converting and look at it in a plot.
...
Backing up to that warning, we were informed that "Increasing adapt_delta above 0.8 may help." The `adapt_delta` parameter ranges from 0 to 1. The `brm()` default is 0.8. In my experience, increasing to 0.9 or 0.99 is often a good place to start. For this model, 0.9 wasn't quite enough, but 0.99 worked. Here's how to do it.
...
No warnings, and our effective samples for $\sigma_0$ improved a bit. Compute the WAIC for both models.
