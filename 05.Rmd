...
```{r, echo = F}
# This fit is too large to cache and upload to Github
...
Unlike with the frequentist multilevel software discussed in the text, **brms** will not yield negative values on the $\sigma$ parameters. This is because the **brms** default is to set a lower limit of zero on those parameters. For example, see what happens when we execute `fit3$model`. 
...
rm(fit8, fit9, fit10, fit11, fit12)
```
...
```{r, echo = F}
# This fit is too large to cache and upload to Github
# save(fit13, file = "fits/fit13.rda")
load("fits/fit13.rda")
```
...
```{r, echo = F}
# This fit is too large to cache and upload to Github
# save(fit14, file = "fits/fit14.rda")
load("fits/fit14.rda")
```
...
```{r, echo = F}
# This fit is too large to cache and upload to Github
# save(fit15, file = "fits/fit15.rda")
load("fits/fit15.rda")
```
...
```{r fit16_18, cache = T, warning = F, message = F, results = "hide"}
fit16 <-
...
```{r fit19, cache = T, warning = F, message = F, results = "hide"}
fit19 <-
  brm(data = medication_pp, 
      family = gaussian,
