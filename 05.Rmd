...
fit5.23 <-
  brm(data = medication_pp, 
      family = gaussian,
      bf(pos ~ 0 + Intercept + time + treat + time:treat + (1 + time |i| id),
         sigma ~ 1 + (1 |i| id)),
      prior = c(prior(normal(150, 50), class = b, coef = Intercept),
                prior(normal(3.5, 0.5), class = Intercept, dpar = sigma),
                prior(normal(0, 25),   class = b),
                prior(student_t(3, 0, 50), class = sd),
...
If you look at the 'sd(sigma_Intercept)' line in the output, there appears to be a bit of variability in that parameter across participants. If the summary appears small compared to those for 'sd(Intercept)' and 'sd(time)', just remember that only 'sd(sigma_Intercept)' is on the log scale. It might be easier to appreciate this variability in a coefficient plot.

```{r}
posterior_samples(fit5.23) %>% 
  pivot_longer(contains("r_id__sigma")) %>% 
  mutate(sigma_id = exp(b_sigma_Intercept + value)) %>% 
  select(name, sigma_id) %>% 
  ...
  ggplot(aes(x = rank, y = sigma_id, ymin = .lower, ymax = .upper)) +
  geom_rect(xmin = -Inf, xmax = Inf,
            ymin = VarCorr(fit5.19)$residual$sd[3],
            ymax = VarCorr(fit5.19)$residual$sd[4],
            fill = "grey67") +
  geom_hline(yintercept = VarCorr(fit5.19)$residual$sd[1]) +
  geom_pointrange(fatten = 1.25) +
  scale_x_continuous("rank ordered participants", breaks = NULL) +
  scale_y_continuous(expression(sigma), limits = c(0, NA)) +
  theme(panel.grid = element_blank())
```

Each of the vertical point intervals is one of the estimates of $\sigma_\text{id}$, based on `fit5.23`. For convenience, they've been rank ordered by their posterior means. Do you see the horizontal gray ribbon and black line in the backgroud? Those correspond to the 95% intervals and posterior mean of the sole $\sigma$ parameter from `fit5.19`, the corresponding conventional multilevel growth model. Those point intervals show there's a hole lot of heterogeneity in a parameter previously set to equality across participants.

```{r}
posterior_summary(fit5.19) %>% 
  data.frame() %>% 
...
```{r}
c <-
  tibble(intercept = coef(fit5.23)$id[, 1, "Intercept"],
         slope     = coef(fit5.23)$id[, 1, "time"],
         sigma     = coef(fit5.23)$id[, 1, "sigma_Intercept"] %>% exp())

c
```

Plot the relations among the random parameters.
...
p1 <-
  ...
  geom_point() +
  labs(subtitle = expression(rho==-0.29*', 95% CI ['*-0.53*', '*-0.03*']'))

p2 <-
  c %>% 
  ggplot(aes(x = intercept, y = sigma)) +
  geom_point() +
  labs(subtitle = expression(rho==0.50*', 95% CI ['*0.28*', '*0.68*']'),
       y = expression(log(sigma)))

p3 <-
  c %>% 
  ggplot(aes(x = slope, y = sigma)) +
  geom_point() +
  labs(subtitle = expression(rho==0.19*', 95% CI ['*-0.07*', '*0.44*']'),
       y = expression(log(sigma)))
...
fit5.24 <-
  brm(data = medication_pp, 
      family = gaussian,
      bf(pos   ~ 0 + Intercept + time + treat + time:treat + (1 + time |i| id),
         sigma ~ 0 + Intercept + time + treat + time:treat + (1 + time |i| id)),
      prior = c(prior(normal(150, 50), class = b, coef = Intercept),
                prior(normal(3.5, 0.5), class = b, coef = Intercept, dpar = sigma),
                prior(normal(0, 25), class = b),
...
```{r, warning = F, message = F}
fit5.23 <- add_criterion(fit5.23, "waic")
fit5.24 <- add_criterion(fit5.24, "waic")

loo_compare(fit5.19, fit5.23, fit5.24, criterion = "waic") %>% 
  print(simplify = F)

model_weights(fit5.19, fit5.23, fit5.24, weights = "waic") %>% 
  round(digits = 3)
```

```{r}
model_weights(fit5.19, fit5.23, fit5.24, weights = "stacking") %>% 
  round(digits = 3)
```

```{r}
loo_compare(fit5.19, fit5.23, criterion = "waic") %>% 
  print(simplify = F)

model_weights(fit5.19, fit5.23, weights = "waic") %>% 
  round(digits = 3)

model_weights(fit5.19, fit5.23, weights = "stacking") %>% 
  round(digits = 3)
```
...
rm(reading_pp, n, lkj, wages_pp, post, nd, f, wages_small_pp, fit5.6, v, fit5.7, fit5.8, fit5.9, fit5.10, unemployment_pp, lines, arrow, medication_pp, fit5.19, text, fit5.20, fit5.21, fit5.22, fit5.23, c, fit5.24)
...
```{r}
tibble(name = str_c("fit5.", c(19, 23))) %>% 
  mutate(fixef = map(name, ~get(.) %>% 
                       fixef() %>% 
                       data.frame() %>% 
                       rownames_to_column("parameter"))) %>% 
  ...
  mutate(parse = case_when(
    parameter == "Intercept" ~ "gamma[0][0]",
    parameter == "time" ~ "gamma[1][0]",
    parameter == "treat" ~ "gamma[0][1]",
    parameter == "time:treat" ~ "gamma[1][1]",
    parameter == "sigma_Intercept" ~ "eta[0][0]"
  )) %>% 
  
  ggplot(aes(x = Estimate, xmin = Q2.5, xmax = Q97.5, y = name)) +
  geom_pointrange() +
  ...
  theme(axis.ticks.y = element_blank(),
        panel.grid = element_blank(),
        strip.text = element_text(size = 11)) +
  facet_wrap(~parse, scales = "free_x", labeller = label_parsed, ncol = 5)
```
